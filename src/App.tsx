...
function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [loadedCategories, loadedNotes, savedTheme, savedFont] = await Promise.all([api.listCategories(), api.listNotes("all"), api.getSetting("theme"), api.getSetting("font")]);
  ...
  const openNote = async (id: string) => {
    ...
    try {
      ...
      if (note) {
        ...
        currentRef.current = documentNote; setCurrent(documentNote); setSourceMode(false); setMobileScreen("note");
  ...
  const createNote = () => {
    ...
    currentRef.current = note; setCurrent(note); setOverflow(false); setMobileScreen("note"); setSourceMode(false);
    window.setTimeout(() => document.querySelector<HTMLElement>(".paper-editor")?.focus(), 50);
  };

  const switchPlace = (place: ActivePlace) => {
    setActivePlace(place); setSearch(""); setCurrent(null); currentRef.current = null; setOverflow(false); setEditorExpanded(false); setMobileScreen("notes");
  };
  ...
  return (
    <main className={`app ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${editorExpanded ? "editor-expanded" : ""} ${mobileScreen === "categories" ? "show-categories" : mobileScreen === "notes" ? "show-notes" : "show-note"}`}>
      ...
      <section className="note-pane">
        {activePlace === "settings" ? <Settings onExport={exportNotebook} theme={theme} onThemeChange={changeTheme} font={font} onFontChange={changeFont} /> : current ? <>
          ...
          {current.deletedAt ? <div className="trashed-note"><div><Icon name="trash" size={20} /><strong>This note is in Trash</strong><span>It will be permanently removed after 30 days.</span></div><button className="primary-button" onClick={() => void restoreCurrent()}>Restore Note</button></div> : <>
            <div className={`document ${sourceMode ? "source" : "paper"}`}>
              {sourceMode ? <div className="writing-surface source-surface"><MarkdownEditor value={current.body} onChange={(body) => changeCurrent({ body })} onReady={(view) => { editor.current = view; }} /></div> : <PaperEditor value={current.body} onChange={(body) => changeCurrent({ body })} onInsertImage={addImage} />}
            </div>
          </>}
...
function Settings({ onExport, theme, onThemeChange, font, onFontChange }: { onExport: () => void; theme: ThemeId; onThemeChange: (theme: ThemeId) => void; font: FontId; onFontChange: (font: FontId) => void }) {
  return <div className="settings"><header><span className="eyebrow">Notebook</span><h1>Settings</h1><p>Your notes live on this device and are always yours to take with you.</p></header><section className="theme-section"><div><strong>Editor appearance</strong><span>Choose a comfortable colour palette for writing.</span></div><div className="theme-grid" role="group" aria-label="Editor appearance">{themes.map((option) => <button className={theme === option.id ? "theme-card selected" : "theme-card"} key={option.id} onClick={() => onThemeChange(option.id)} aria-pressed={theme === option.id}><span className="theme-preview" style={{ background: option.colors[0] }}><i style={{ background: option.colors[1] }} /><b style={{ background: option.colors[2] }} /></span><span><strong>{option.name}</strong><small>{option.description}</small></span>{theme === option.id && <Icon name="check" size={15} />}</button>)}</div></section><section className="font-section"><div><strong>Writing font</strong><span>Use a clean sans serif throughout your notebook.</span></div><div className="font-options" role="group" aria-label="Writing font">{fonts.map((option) => <button className={font === option.id ? "font-option selected" : "font-option"} key={option.id} onClick={() => onFontChange(option.id)} aria-pressed={font === option.id}><span>{option.name}</span><small>{option.description}</small>{font === option.id && <Icon name="check" size={14} />}</button>)}</div></section><section><div><strong>Export your notebook</strong><span>Create a ZIP of readable Markdown files, organized by folder.</span></div><button className="secondary-button" onClick={onExport}><Icon name="archive" size={17} />Export Markdown</button></section><section><div><strong>Private by default</strong><span>No account, server, or tracking is needed to write and organize your notes.</span></div><span className="status-pill"><i />Stored locally</span></section></div>;
}
